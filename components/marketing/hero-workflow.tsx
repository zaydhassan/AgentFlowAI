...
// Minimal technical chain: Trigger → Intelligence → Tools → Action.
// Electric blue carries the flow; neutral tiles frame the ends.
const NODES: Node[] = [
  { icon: "Mail", label: "Trigger", sub: "gmail · new.message", color: "#8B98A8" },
  { icon: "Sparkles", label: "AI Agent", sub: "claude · reasoning", color: "#2196F3", running: true },
  { icon: "Wrench", label: "Tools", sub: "mcp · tool.call", color: "#42A5F5" },
  { icon: "MessageSquare", label: "Action", sub: "slack · post #ops", color: "#8B98A8" },
];
  ...
  return (
    ...
    >
      <motion.div style={innerStyle} initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.3, ease: [0.22, 1, 0.36, 1] }}>
        <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="w-full h-auto overflow-visible" role="img" aria-label="Example AI agent workflow: trigger, AI agent, tools, action">
          <defs>
            <linearGradient id="wf-stroke" x1="0" y1="0" x2="0" y2="1">
          ...
          {edges.map((_, i) => (
            <circle key={`m${i}`} r={3.2} fill="#42A5F5" style={{ filter: "drop-shadow(0 0 4px rgba(33,150,243,0.6))" }}>
              ...
              <foreignObject key={n.label} x={p.x} y={p.y} width={NW} height={NH}>
                ...
                >
                  <div className="relative h-full w-full">
                    {/* Glow halo — only the running node gets one accent point. */}
                    {n.running && (
                      <motion.span
                        className="pointer-events-none absolute -inset-2 rounded-xl"
                        style={{ background: `radial-gradient(circle at center, ${n.color}44, transparent 70%)`, filter: "blur(8px)" }}
                        animate={{ opacity: [0.5, 0.8, 0.5] }}
                        transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }}
                      />
                    )}
                    {/* Card */}
                    <div className="relative flex h-full w-full items-center gap-2.5 rounded-lg border border-border bg-surface-2 px-3">
                      <span
