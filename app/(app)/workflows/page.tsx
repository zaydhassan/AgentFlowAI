...
const statusTone = (s: WorkflowStatus) =>
...
export default function WorkflowsPage() {
  ...
  return (
    <div className="animate-float-up">
      <PageHeader
        title="Workflows"
        description={`${workflows.length} workflows · ${workflows.filter((w) => w.status === "active").length} active`}
        actions={
          <>
            <Button variant="secondary" size="sm"><Icon name="Upload" className="h-3.5 w-3.5" /> Import</Button>
            <Link href="/ai"><Button size="sm" variant="ai"><Icon name="Wand2" className="h-3.5 w-3.5" /> New with AI</Button></Link>
          </>
        }
      />
        ...
        {list.map((w) => (
          <Link key={w.id} href={`/workflows/${w.id}`}>
            <Card className="card-hover h-full p-4">
              ...
              <p className="mt-1 line-clamp-2 text-xs text-fg-muted">{w.description}</p>
              ...
              <div className="mt-4 flex items-center justify-between border-t border-border pt-3">
                <div className="flex items-center gap-3 text-[11px] text-fg-subtle">
                  <span className="flex items-center gap-1"><Icon name="Clock" className="h-3 w-3" /> {relativeTime(w.lastRun)}</span>
      ...
      {list.length === 0 && (
        <div className="rounded-xl border border-dashed border-border py-16 text-center">
          ...
          <p className="mt-3 text-sm text-fg-muted">No workflows match your filters.</p>
        </div>
      )}
    </div>
  );
